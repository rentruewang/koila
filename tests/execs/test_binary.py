...
def test_concat_stream_next(block_frame, concat_frame, size):
    stream = ZipExec(
        block_frame.iterate(batch_size=size), concat_frame.iterate(batch_size=size)
    )

    for result, lhs, rhs in zip(
        stream,
        block_frame.iterate(batch_size=size),
        concat_frame.iterate(batch_size=size),
    ):
...
def test_join_stream_next(block_frame, joinable_frame, join_batch_size):
    stream = MatrixJoinExec(
        block_frame.iterate(batch_size=join_batch_size),
        joinable_frame,
        on="i1d",
