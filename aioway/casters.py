...
class Caster[T]:
    ...
    """
    The aliases of the underlying type used in ``astype``.
    """

    klasses: Sequence[type[T]]
    """
    The sub classes of the caster.
    """

    matrix: Sequence[Sequence[Callable[[T], T] | None]]
    """
    ...
    def __getitem__(self, idx: tuple[str, str]) -> Callable[[T], T] | None:
        """
    ...
    def __klass_order_unchecked(self, klass: type[T]) -> int:
        return self.klasses.index(klass)

    def __alias_of_unchecked(self, klass: type[T]) -> str:
        index = self.__klass_order(klass)
        return self.aliases[index]

    def __class_name_unchecked(self, name: str) -> type[T]:
        index = self.__alias_order(name)
    ...
    class_name = _checked(__class_name_unchecked, IndexError)


class Castable(ABC):
    """
    A mixin that allows the use of ``astype`` like numph.
    """

    @classmethod
    @abc.abstractmethod
    def _caster(cls) -> Caster[Self]:
        """
    ...
    def astype(self, to: LiteralString, /) -> Self:
        """
